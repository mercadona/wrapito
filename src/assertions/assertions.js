...
const toMatchNetworkRequests = mockedResponses => {
...
const createResponsesNotBeingUsedMatcher = mockedResponses => {
  const getResponseBeingUsedChecker = mockedResponse => utilizedResponse =>
    !deepEqual(mockedResponse, utilizedResponse)
  const getResponsesNotBeingUsed = mockedResponse =>
    getDeepUtilizedResponses().every(
      getResponseBeingUsedChecker(mockedResponse),
    )

  const responsesNotBeingUsed = formatMockedResponses(mockedResponses)
    .filter(getResponsesNotBeingUsed)
    .map(({ multipleResponses, ...restOfResponse }) => {
      const hasMultipleResponses =
        multipleResponses && multipleResponses.length > 0
...
const createMissingMockedResponseMatcher = requestsMissingResponse => {
  const message = () =>
    createErrorMessage(ERRORS.MISSING_MOCKED_RESPONSE, requestsMissingResponse)
...
const createErrorMessage = (error, responses) =>
  red(
    `Expected mocked responses to match the network requests but ${error}\n\n${prettyFormat(
      responses,
    )}`,
  )
